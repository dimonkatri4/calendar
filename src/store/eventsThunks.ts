...
export const addEventThunk = (newEvent: INewEventData) => async (dispatch: AppDispatch) => {
    try {
        dispatch(toggleIsFetching(true))
        const data = await eventsApi.addEvent(newEvent)
        dispatch(toggleIsFetching(false))
...
export const updateEventThunk = (updateEventData: IUpdateEvent) => async (dispatch: AppDispatch) => {
    try {
        dispatch(toggleIsFetching(true))
        const data = await eventsApi.updateEvent(updateEventData.id, updateEventData.data)
        dispatch(toggleIsFetching(false))
